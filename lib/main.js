...
class GpioInstance extends InstanceBase {
	...
	setupGPIO() {
		...
		for (const pinNumber of this.active_output_pins) {
			const gpioPin = pinNumber + this.config.base
			console.log(`Pin_num: ${pinNumber}(${gpioPin}) OUTPUT`)
			try {
				this.active_output_pin_objects.set(pinNumber, new Gpio(gpioPin, 'out'))
			} catch (err) {
		...
		for (const [pinNumber, config] of this.active_input_pins.entries()) {
			...
			console.log(`Pin_num: ${pinNumber}(${gpioPin}) INPUT`)
			...
			try {
				const gpioHandle = new Gpio(gpioPin, 'in', trigger_type, {
